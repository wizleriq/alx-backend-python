...
class IsParticipantOfConversation(permissions.BasePermission):
    ...
    - Participants can only send, view, update, and delete their own messages
    """

    def has_permission(self, request, view):
        # Explicit check: user must be logged in
        return request.user and request.user.is_authenticated

    def has_object_permission(self, request, view, obj):
        # If the object is a Conversation (has participants field)
        if hasattr(obj, "participants"):
            return request.user in obj.participants.all()

        # If the object is a Message (belongs to a conversation)
        if hasattr(obj, "conversation"):
            # First check: is the user a participant in the conversation?
            if request.user not in obj.conversation.participants.all():
                return False

            # If user is editing (PUT/PATCH) or deleting (DELETE)
            if request.method in ["PUT", "PATCH", "DELETE"]:
                # Only the sender of the message can edit/delete
                return obj.sender == request.user

            # Otherwise (GET or POST), allow
            return True

        # If it's neither a Conversation nor a Message, deny
        return False
